...
image = cv2.imread("shapes.png")  #say to the computer read the img in the adress
...
contours, hierarchy = cv2.findContours(thresh_img, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)  #find the contours and save only the corners
...
for i, contour in enumerate(contours):
    ...
    approx = cv2.approxPolyDP(contour, epsilon, True)  #this line and the previous make the cv2 read imperfections, the True say that they are closed shapes

    cv2.drawContours(image, contour, 0, (0, 255, 0), 4)

    x, y, w, h = cv2.boundingRect(approx) #find the cordinates of the shapes
    x_mid = int(x + (w / 3))  #returns the center of the x
    y_mid = int(y + (w / 1.5))  #returns the center of the y
    #originaly the 3 and the 1.5 where both 2, but the text was a little bit dislocate, so it's 3 and 1.5
